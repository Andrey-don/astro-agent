...
CONTENT_PLAN_FILE = os.path.join(PROJECT_ROOT, "content-plan.md")


def append_topics_to_plan(topics: list) -> None:
    """Добавляет новые темы в content-plan.md по разделам."""
    with open(CONTENT_PLAN_FILE, "r", encoding="utf-8") as f:
        content = f.read()

    # Группируем новые темы по разделам
    by_section: dict = {}
    for section, topic in topics:
        by_section.setdefault(section, []).append(topic)

    # Считаем максимальный номер темы в файле
